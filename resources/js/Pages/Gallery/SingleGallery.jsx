...
const GalleryComponent = ({ gallery, inModal }) => {
    const increaseViews = () => {
        axios.post(route("gallery.increaseViews", { gallery: gallery.id }));
    };
        ...
        <div className={`justify-center w-full ${inModal ? "p-3" : "p-0"}`}>
            <div className="flex items-start">
                <div className="mr-5 flex flex-col items-center flex-shrink-0">
                    ...
                    >
                        <img
                            src={gallery?.streamer?.profile_picture}
                ...
                <div>
                    ...
                    <div className="flex items-center flex-wrap md:space-x-2 mt-1">
                        ...
                        >
                            @{gallery?.streamer?.username}
                        </Link>

                        <Link
                            href={route("gallery.browse", {
                                videocategory: gallery?.category.id,
                        ...
                        >
                            ...
                            <span>{gallery?.category?.category}</span>
                        </Link>
                        ...
                        <span className="text-gray-600 inline-flex items-center space-x-1 dark:text-zinc-100 text-sm mr-2   ">
                            ...
                            {gallery?.views === 1
                                ? __("1 view")
                                : __(":viewsCount views", {
                                      viewsCount: gallery?.views,
                                  })}
                        </span>

                        {gallery?.free_for_subs === "yes" && gallery?.price !== 0 && (
...
export default function SingleVideo({ gallery, inModal = false }) {
    if (inModal) {
        return <GalleryComponent gallery={gallery} inModal={true} />;
    } else {
        return (
            <AuthenticatedLayout>
                <Head title={gallery?.title} />
                <GalleryComponent gallery={gallery} inModal={false} />
            </AuthenticatedLayout>
        );
    }
}
